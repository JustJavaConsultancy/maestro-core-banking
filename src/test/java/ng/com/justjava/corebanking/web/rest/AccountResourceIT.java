...
import static ng.com.justjava.corebanking.web.rest.AccountResourceIT.TEST_USER_LOGIN;
...
public class AccountResourceIT {
    ...
    @Test
    @Transactional
    @WithMockUser("change-lost-password")
    public void testChangeLostPassword() throws Exception {
        User user = new User();
        String currentPassword = RandomStringUtils.random(60);
        user.setPassword(passwordEncoder.encode(currentPassword));
        user.setLogin("change-lost-password");
        user.setEmail("change-lost-password@example.com");
        userRepository.saveAndFlush(user);

        Profile profile = new Profile()
            .profileID(UPDATED_PROFILE_ID)
            .phoneNumber("change-lost-password")
        ...
        restAccountMockMvc.perform(post("/api/account/changelostpassword")
                .contentType(MediaType.APPLICATION_JSON)
                .content(TestUtil.convertObjectToJsonBytes(new LostPasswordDTO("change-lost-password", currentPassword, "new password")))
            )
            .andExpect(status().isOk());

        User updatedUser = userRepository.findOneByLogin("change-lost-password").orElse(null);
        assertThat(passwordEncoder.matches("new password", updatedUser.getPassword())).isTrue();
    }
    ...
    public void testChangePasswordByAdmin() throws Exception {
        ...
        String currentPassword = RandomStringUtils.random(60);
        user.setPassword(passwordEncoder.encode(currentPassword));
        user.setLogin("change-lost-password-by-admin");
        user.setEmail("change-lost-password-by-admin@example.com");
        userRepository.saveAndFlush(user);

        Profile profile = new Profile()
            .profileID(UPDATED_PROFILE_ID)
            .phoneNumber("change-lost-password-by-admin")
            .gender(UPDATED_GENDER)
            .dateOfBirth(UPDATED_DATE_OF_BIRTH)
            .address(UPDATED_ADDRESS)
            .photo(UPDATED_PHOTO)
        ...
        restAccountMockMvc.perform(post("/api/admin/password/change")
                .contentType(MediaType.APPLICATION_JSON)
                .content(TestUtil.convertObjectToJsonBytes(new LostPasswordDTO("change-lost-password-by-admin", currentPassword, "new password")))
            )
            .andExpect(status().isOk());

        User updatedUser = userRepository.findOneByLogin("change-lost-password-by-admin").orElse(null);
        assertThat(passwordEncoder.matches("new password", updatedUser.getPassword())).isTrue();
    }
