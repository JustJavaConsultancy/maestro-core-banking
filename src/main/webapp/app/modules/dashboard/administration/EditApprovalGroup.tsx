...
export default function ApprovalGroup(props:any) {
    ...
    const sendForApproval = async () => {
        ...
        try {
            setLoading(true)
            const submit = await Axios.put('/api/approval-groups', data);
            console.error(submit);
            alert("Approval Updated Successfully");
            props.history.push("/approval-groups");
        } catch (error) {
            if(error.response) {
    ...
    const getGetDetail = async () => {
        try {
            const details = await Axios.get(`api/approval-groups/${props.match.params.id}`);
            setSelectedUser(details.data.data.phoneNumbers)
            setGroupName(details.data.data.name)
            setTimeout(() => {
                getUsers(0);
            },500)
    ...
    return(
        <Layout>
            <InnerLayout dontfilter = {true} show = {true} title = 'Edit Approval Group' path = 'Edit-approval-group'>
                <div className = "approval-gorup-selector">
                    <label style = {{width: "900px"}}>
                        Create Group
                        <input value = {groupName} onChange = {e => setGroupName(e.target.value)} className = "custom-input1" placeholder = "Enter Group Name" />
                    </label>
                </div>
                <br/>
                <div>
                ...
                <div style = {{display: "flex", flexDirection: "column",  justifyContent: "center"}} className = "approval-gorup-selector">
                    ...
                    <div style = {{display: "flex", justifyContent: "center", alignItems: "center"}}>
                        <button onClick = {sendForApproval} disabled = {loading} className = 'viewreport'>{loading? "Loading..." : "Edit Group"}</button>
                    </div>
                </div>
            </InnerLayout>
        </Layout>
