...
public class FCMService {
    ...
    public FCMService(Utility utility) {
        this.utility = utility;
    }

    private String sendAndGetResponse(Message message, String channel) throws InterruptedException, ExecutionException {
        FirebaseApp app = FirebaseApp.getInstance("pouchii");
        if(utility.checkStringIsValid(channel)) app = FirebaseApp.getInstance(channel.trim().toLowerCase());
            return FirebaseMessaging.getInstance(app).sendAsync(message).get();
    }
	...
	public void sendMessageWithoutData(PushNotificationRequest request)
		...
		String response = sendAndGetResponse(message, request.getChannel());
